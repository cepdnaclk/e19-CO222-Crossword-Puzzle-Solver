...
struct puzzle checkVertical(int horizontalPosition, int verticalPosition, struct puzzle myMatrix, char currentWord[10]);
struct puzzle checkHorizontal(int horizontalPosition, int verticalPosition, struct puzzle myMatrix, char currentWord[10]);
void printValidPuzzle(struct puzzle solution, int gridLength);
void fillWords(struct puzzle myPuzzle, char words[100][10], int wordIndex, int gridLength);
...
int main() {
    ...
    while(1) {
        ...
        if(blankLine == 0) {
            ...
            if((rows == 0) && (strlen(puzzle[rowNum]) == 0)) {
                invalid=1;

            // If the other line is a blank, it finishes taking the inputs for the puzzle
            } else if(strlen(puzzle[rowNum]) == 0) {
                blankLine=1;

            // Check the current row of the puzzle if there are invalid characters
            } else if(strlen(puzzle[rowNum])) {

                // Direct along the row
                for(int columnIndex = 0; columnIndex < strlen(puzzle[rowNum]); columnIndex++) {

                    // Introduce condition to identify invlid characters
                    if(((puzzle[rowNum][columnIndex] != '#') && (puzzle[rowNum][columnIndex] != '*')) && (isalpha(puzzle[rowNum][columnIndex]) == 0)) {
                        invalidcount++;
                    }

                    // Count for hash signes
                    if(puzzle[rowNum][columnIndex] == '#') {
                        wordcharsr[rowNum]++;
                        wordcharsc[columnIndex]++;
                        puzzlechars++;
                    }
                }
            ...
            // Introduce condition to recognize possible to fill state
                ...
                for(iteration1 = 0; iteration1 < 100; iteration1++) {
                    for(iteration2 = 0; iteration2 < 100; iteration2++) {
                        draftPuzzle.matrix[iteration1][iteration2] = puzzle[iteration1][iteration2];
                    }
                }

                // By trying the all combinations, fill the pizzle
                fillWords(draftPuzzle, words, 0, strlen(puzzle[0]));

                // If no possible solutions can be found the program reaches here
                printf("IMPOSSIBLE");
